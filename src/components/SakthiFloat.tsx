...
export default function SakthiFloat() {
  const { language, t, activeRole } = useApp();
  ...
  const activeTab = searchParams.get('tab') || (activeRole === 'farmer' ? 'myfarm' : 'advisory');
  ...
  const getVoiceAlerts = () => {
    ...
    if (pathname === '/dashboard') {
      if (activeTab === 'myfarm') {
        briefs.en = {
          title: "My Farm Central Hub Guide",
          speech: "Welcome to My Farm dashboard. Here you can track your current crop stage, monitor expected harvest dates, check weather alerts, and review your farm summary including expenses, income and net profits."
        };
        briefs.ta = {
          title: "என் பண்ணை வழிகாட்டி",
          speech: "என் பண்ணை பக்கத்திற்கு உங்களை வரவேற்கிறோம். இங்கு உங்களது பயிர் நிலை, அறுவடை தேதிகள், வானிலை எச்சரிக்கைகள், மற்றும் பண்ணை கணக்குகளான வரவு, செலவு மற்றும் லாபத்தை எளிதாகக் கண்காணிக்கலாம்."
        };
        briefs.hi = {
          title: "मेरा खेत गाइड",
          speech: "मेरे खेत डैशबोर्ड में आपका स्वागत है। यहाँ आप अपनी फसल के विकास के चरण, कटाई की तारीखें, मौसम की चेतावनियाँ और अपने खेत के खर्च, आय तथा शुद्ध लाभ की जानकारी देख सकते हैं।"
        };
      } else if (activeTab === 'advisory' || activeTab === 'home') {
        briefs.en = {
      ...
      briefs.ta = {
        ...
        speech: "டெலிவரி முகப்பு பலகை. காய்கறிகளை ஏற்றிச்செல்ல வழிகளைத் தேர்ந்தெடுக்கவும். 'வழியை ஏற்றுக்கொள்' பொத்தானைத் தட்டவும். டெலிவரி முடிந்ததும் 'விநியோகம் முடிந்தது' எனப் பதிவு செய்யவும்."
      };
      briefs.hi = {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed inset-x-0 bottom-0 bg-white/95 dark:bg-[#111714]/95 backdrop-blur-md border-t border-[#e6eae7] dark:border-primary-950/20 z-50 p-5 rounded-t-3xl shadow-2xl animate-slide-up max-w-lg mx-auto pb-8 md:pb-6">
