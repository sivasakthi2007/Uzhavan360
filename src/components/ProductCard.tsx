...
export default function ProductCard({ product, onBuyClick, isBuyable = false }: ProductCardProps) {
  ...
  return (
    <div className="rounded-2xl border border-[#e6eae7] dark:border-primary-950/20 bg-white dark:bg-[#111714] overflow-hidden flex flex-col shadow-sm hover-card">
      {/* Product Image */}
        ...
        <div>
          <div className="flex items-start justify-between gap-1">
            <div className="flex flex-wrap gap-1 mb-2">
              {product.isVerifiedFarmer && <TrustBadge type="verified" customText={t('verified_farmer') || 'Verified Farmer'} />}
              {product.isRecommended && <TrustBadge type="recommended" />}
              {product.farmerRating >= 4.7 && <TrustBadge type="top_rated" />}
              {product.distanceKm < 15 && <TrustBadge type="nearby" />}
              {product.isNew && <TrustBadge type="new" />}
            </div>
